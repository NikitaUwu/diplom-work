...
sequenceDiagram
    ...
    FE->>BE: Upload image
    BE->>FS: Save original image
    BE->>DB: Create chart
    BE->>DB: Create processing_job queued
    BE->>DB: Create mqtt_message out pending
    BE->>BE: Signal outbox dispatcher

    BE->>DB: Load pending outbox message
    BE->>MQTT: Publish charts/process/request
    BE->>DB: Mark mqtt_message published
    BE->>DB: Mark job published

    W->>MQTT: Subscribe charts/process/request
    MQTT-->>W: Deliver request
    W->>W: Put job into local queue
    W->>MQTT: Publish charts/process/accepted
    MQTT-->>BE: Deliver accepted
    BE->>DB: Insert inbox event
    BE->>DB: Mark job/chart processing and create lease

    loop While pipeline is running
        W->>MQTT: Publish charts/process/heartbeat
        MQTT-->>BE: Deliver heartbeat
        BE->>DB: Extend leased_until
    end

    alt Success
        W->>FS: Write data.json and artifacts
        W->>MQTT: Publish charts/process/completed
        MQTT-->>BE: Deliver completed
        BE->>DB: Insert inbox event
        BE->>FS: Read data.json
        BE->>DB: Mark job/chart done
        FE->>BE: Load result
    else Worker error
        W->>FS: Write failed data.json if possible
        W->>MQTT: Publish charts/process/failed
        MQTT-->>BE: Deliver failed
        BE->>DB: Insert inbox event
        BE->>DB: Retry or mark job/chart error
        FE->>BE: Load error and artifacts
    else Worker disappeared
        BE->>DB: Lease monitor finds expired job
        BE->>DB: Retry or mark processing_lease_expired
    end
```
