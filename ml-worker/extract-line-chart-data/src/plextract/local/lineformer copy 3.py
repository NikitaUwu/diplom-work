...
class LineFormer:
    ...
    def _preprocess_image(self, img: np.ndarray, results_path: Path):
        """
        Экстремальная очистка: удаляем всё тонкое (сетку, пунктиры), оставляем только жирные линии.
        """
        ...
        # Бинаризация (черные линии становятся белыми)
        _, thresh = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)

        # 1. УБИВАЕМ ТОНКИЕ ЛИНИИ (сетку и диагонали)
        # Ядро 3x3 сотрет всё, что тоньше 3 пикселей. Жирные линии графиков просто похудеют.
        kernel_erode = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
        eroded = cv2.erode(thresh, kernel_erode, iterations=1)
        
        # Возвращаем толщину оставшимся (жирным) линиям
        dilated = cv2.dilate(eroded, kernel_erode, iterations=1)

        # 2. СКЛЕИВАЕМ РАЗРЫВЫ (если жирная линия чуть-чуть порвалась)
        kernel_close = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
        clean_lines = cv2.morphologyEx(dilated, cv2.MORPH_CLOSE, kernel_close, iterations=1)

        # Формируем финальное изображение (черные линии на белом фоне)
        final_input = cv2.cvtColor(clean_lines, cv2.COLOR_GRAY2BGR)
        final_input = cv2.bitwise_not(final_input)

        # Отсекаем верхние и нижние 12% картинки (там где рамки и текст осей)
        h, w = final_input.shape[:2]
        final_input[:int(h*0.12), :] = [255, 255, 255]
        final_input[int(h*0.88):, :] = [255, 255, 255]

        # Сохраняем для дебага, чтобы ты мог посмотреть в папке, как круто очистился график!
